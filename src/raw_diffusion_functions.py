# ### functions last updated on 28.09 from ear5_palett_bias correction
...
class BaseModel():
    ...
    def train(self):
        while self.epoch <= self.n_epochs: #and self.iter <= self.n_iter:
            ...
            if self.epoch % 25 == 0:
                output_sampled, _ = diffusion_network.restoration(condition_after_unet.to("cuda"), sample_num=8)
    ...
    def save_network(self, network, network_label):
        ...
        save_path = os.path.join(os.path.join("models", "DDPM_conditional", save_filename))
        if isinstance(network, nn.DataParallel) or isinstance(network, nn.parallel.DistributedDataParallel):
            network = network.module
...
# +
# #%%capture
#from era5_precipitation_preprocessing import GFDL_P_Dataset_1_1
# -
...
gfdl_ = gfdl.data()
bs_valid = 5
dataloader_gfdl = data.DataLoader(gfdl_, batch_size=bs_valid, shuffle=False, drop_last=True,num_workers=2)
